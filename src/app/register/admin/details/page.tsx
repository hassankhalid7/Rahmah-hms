...
function AdminDetailsForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/public/register-institute', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        const errorMessage = errorData?.error || errorData?.message || 'Failed to register institute';
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    className="w-full rounded-2xl border border-[#d0d8cf] bg-[#F7F1E6] px-4 py-2.5 text-sm text-[#1c3c33] outline-none transition focus:border-[#2F6B4F] focus:bg-white focus:ring-2 focus:ring-[#2F6B4F]/15"
                  />
                </div>
