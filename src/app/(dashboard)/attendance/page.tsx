...
export default function AttendancePage() {
            ...
            fetch('/api/attendance')
                ...
                .then(data => {
                    ...
                    const grouped = records.reduce((acc: any, curr: any) => {
                        const key = `${curr.classId}_${curr.date}`;
                        if (!acc[key]) {
                            acc[key] = { 
                                ...
                                count: 0, 
                                present: 0,
                                className: classes.find(c => c.id === curr.classId)?.name || 'Unknown Class'
                            };
                        }
                        acc[key].count++;
    ...
    useEffect(() => {
        if (!classId || view !== 'mark') { setStudents([]); return; }
        setLoading(true);
        
        Promise.all([
            fetch(`/api/classes/${classId}/students`).then(r => r.json()),
            fetch(`/api/attendance?date=${date}&class_id=${classId}`).then(r => r.json())
        ])
        .then(([studentData, attendanceData]) => {
            const existingAttendance = Array.isArray(attendanceData.attendance) ? attendanceData.attendance : [];
            
            setStudents(
                (Array.isArray(studentData) ? studentData : []).map((s: any) => {
                    const saved = existingAttendance.find((a: any) => a.userId === s.userId);
                    return {
                        ...s,
                        status: (saved?.status as Status) || 'present',
                    };
                })
            );
        })
        ...
        <div className="min-h-screen bg-[#FDFBF7] p-4 md:p-8">
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex bg-white rounded-xl border border-[#1c3c33]/6 p-1 self-start">
                        <button 
                            onClick={() => setView('mark')}
                            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${view === 'mark' ? 'bg-[#2F6B4F] text-white' : 'text-[#1c3c33]/40 hover:text-[#1c3c33]'}`}>
                            Mark Attendance
                        </button>
                        <button 
                            onClick={() => setView('history')}
                            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${view === 'history' ? 'bg-[#2F6B4F] text-white' : 'text-[#1c3c33]/40 hover:text-[#1c3c33]'}`}>
                            View History
                        </button>
                    </div>
                ...
                {view === 'mark' ? (
                    ...
                    <div className="bg-white rounded-2xl border border-[#1c3c33]/6 shadow-sm overflow-hidden">
                        <div className="px-5 py-4 border-b border-[#1c3c33]/5 bg-[#FAFAF8]">
                            <h2 className="text-sm font-black text-[#1c3c33]">Previous Saved Attendance</h2>
                        </div>
                        ...
                        {loadingHistory ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-16 text-center gap-2">
                                <span className="text-3xl">📭</span>
                                <p className="text-sm font-bold text-[#1c3c33]/50">No attendance records found</p>
                            </div>
                        ) : (
                            <div className="divide-y divide-[#1c3c33]/5">
                                {history.map((h: any) => (
                                    <div key={h.date} className="px-5 py-4 flex items-center justify-between hover:bg-[#FAFAF8] transition-colors group">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-2xl bg-[#E8F5EE] flex items-center justify-center text-lg shadow-sm group-hover:scale-110 transition-transform">
                                                📅
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-[#1c3c33]">{new Date(h.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                                <p className="text-[10px] text-[#1c3c33]/60 font-black uppercase tracking-widest mt-0.5">
                                                    {h.className}
                                                </p>
                                                <p className="text-[10px] text-[#1c3c33]/35 uppercase tracking-widest font-black mt-1">
                                                    {h.count} Records · {Math.round((h.present / h.count) * 100)}% Attendance
                                                </p>
                                            </div>
                                        </div>
                                        <button 
                                            onClick={() => { setDate(h.date); setClassId(h.classId); setView('mark'); }}
                                            className="px-4 py-2 rounded-xl border border-[#d0d8cf] text-[10px] font-black text-[#1c3c33]/50 hover:border-[#2F6B4F] hover:text-[#2F6B4F] transition-all">
                                            View Details
                                        </button>
                                    </div>
                                ))}
